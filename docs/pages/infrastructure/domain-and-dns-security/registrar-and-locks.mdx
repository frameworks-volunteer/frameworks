...
### Registrar Examples: How Teams Commonly Deploy

These are practical examples you can adapt based on your constraints:

1. **MarkMonitor + Cloudflare DNS**
   - Registrar: MarkMonitor (high-assurance transfer/ownership controls)
   - DNS hosting: Cloudflare
   - Typical setup: registry lock + strict approval workflow at registrar, DNSSEC enabled in Cloudflare, DS at registrar,
     and DNS change alerts in both systems

2. **Route 53 Registrar + Route 53 Hosted Zone**
   - Registrar: Amazon Registrar (via Route 53)
   - DNS hosting: Route 53
   - Typical setup: IAM least privilege, MFA-required admin roles, CloudTrail logging for domain and zone changes,
     DNSSEC signing where available and DS tracking at registrar

3. **Cloudflare Registrar + Cloudflare DNS**
   - Registrar + DNS in one platform
   - Typical setup: hardware-key protected admin access, zone lockdown procedures, DNSSEC enabled by default, and
     change alerts through Cloudflare notifications/webhooks

**Design choice**: Splitting registrar and DNS providers can reduce single-vendor risk, but it also increases operational
complexity. If you split, document exactly who can change registrar settings versus DNS records.

Useful references:

- Cloudflare DNS docs: [Documentation](https://developers.cloudflare.com/dns/)
- MarkMonitor: [Official site](https://www.markmonitor.com/)
...
### Registry Lock Implementation Examples

- **MarkMonitor**: Request managed registry lock with out-of-band approval and named approvers. Validate the callback
  process with your security team before production rollout.
- **Route 53 Registrar**: Confirm transfer lock settings and registrar-level protections for each critical domain.
  Pair with strict IAM boundaries and CloudTrail alerting for domain-related API calls.
- **Cloudflare Registrar**: Confirm transfer lock posture and registrar-level controls, then separately verify zone-level
  permissions and audit logs in Cloudflare DNS.

For all providers, run a quarterly control test: attempt a controlled nameserver/contact change and verify that lock and
approval workflows behave exactly as expected.
